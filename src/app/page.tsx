...
export default function Home() {
  ...
  const handleSymbolSearch = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!foundStock) {
      setSymbolError(
        "준비된 대표 종목 목록에서 찾을 수 없습니다. 예: AAPL, MSFT, TSLA",
      );
      return;
    }
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600">
                종목 코드나 종목 이름으로 검색하고, 자주 보는 종목은 즐겨찾기에
                저장하는 공부용 웹앱입니다.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="예: Apple, Microsoft, Tesla"
                ...
                <div className="mt-4 overflow-hidden rounded-md border border-slate-200">
                  {filteredStocks.length > 0 ? (
                    ...
                    <div className="px-4 py-5 text-sm text-slate-500">
                      검색 결과가 없습니다. 예: Apple, Microsoft, Tesla
                    </div>
                  )}
                </div>
              </div>
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-bold text-slate-950">
                {selectedStock.name}{" "}
                <span className="text-slate-400">/ {selectedStock.symbol}</span>
              </h2>
              <p className="mt-2 text-sm text-slate-500">
          ...
          {quoteLoading ? (
            <div className="mt-5 rounded-md border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-medium text-blue-900">
              {selectedStock.symbol} 가격 정보를 조회하고 있습니다.
            </div>
          ) : null}
